...
from app.controllers.order_controller import create_order, create_order_detail, add_garment, add_service
import datetime
...
def get_order_detail_endpoint(order_id):
    try:
        order = create_order_detail(order_id)
        return jsonify({
            "msg": "Detalle de orden obtenido un error", "order":order
        }), 200
    except Exception as e:
        return jsonify({
            "msg": "Ocurrio un error", "error":e
        }), 500
